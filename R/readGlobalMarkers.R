readGlobalMarkers <- function(inDir_v, dirs_v, grep_v = "2x|3xNR|3xR|4x|d80|Ent|NT|PTX|3x|3xR\\+4x|Unt", toRead_v = NULL) {
  #' Read Global Markers
  #' @description
  ...
  #' @param toRead_v used to grep which files to read in.
  ...
  subFiles_lsv <- sapply(dirs_v, function(x) {
    y <- list.files(file.path(inDir_v, x))
    if (!is.null(toRead_v)) y <- y[grep(toRead_v, y)]
